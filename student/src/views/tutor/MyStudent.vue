  ...
  <div>
    <TeacherLayOut>
      <a-card class="student-card" :loading="loading">
        <template #title>
          <a-avatar :src=" 'http://8.130.65.99:8002/' + student.photo" size="large" v-for="student in students" :key="student.id" />
        ...
        <a-descriptions bordered v-for="student in students" :key="student.id">
          <a-descriptions-item label="姓名">{{ student && student.username }}</a-descriptions-item>
          <a-descriptions-item label="性别">{{ student && student.gender }}</a-descriptions-item>
